...
import IPython
from .simgr_helper import overflow_detect_filter, hook_win, hook_four, overflow_rpb_detect_filter
import logging
...
def checkOverflow(binary_name, inputType="STDIN"):
    ...
    return run_environ
def off_by_one_rbp(binary_name, inputType="STDIN"):
	extras = {
		so.REVERSE_MEMORY_NAME_MAP,
		so.TRACK_ACTION_HISTORY,
		so.SYMBOL_FILL_UNCONSTRAINED_MEMORY,
	...
	try:
		@timeout_decorator.timeout(120)
		def exploreBinary(simgr):
			simgr.explore(find=lambda s: "type" in s.globals, step_func=overflow_rpb_detect_filter)
		exploreBinary(simgr)
		if "found" in simgr.stashes and len(simgr.found):
			end_state = simgr.found[0]
			run_environ["type"] = end_state.globals["type"]
